...
def network_broadcast_address(subnet_mask, hosts_amount, subnet_amount, ip_address):
    ...
    if (octet_addition > 1):
        if(int(network_id.split(".")[-2]) < (255 - octet_addition)):
            ...
            next_subnet_id = network_id.split(".")[:1] + [(str(int(octet_addition)))] + [str(0)]+ [str(0)]
            next_subnet_id = joinList(next_subnet_id)
    elif(int(network_id.split(".")[-1]) < (255 - hosts_amount)):
    ...
    first_subnet_broadcast = ''.join(next_subnet_id).split(".")[:2] + [str((int(''.join(next_subnet_id).split(".")[-2]) - 1))] + [255]
    ...
    print()
    print(f'Second subnet ID: {next_subnet_id}')
    ...
    if (octet_addition > 1):
        ...
        next_subnet_broadcast = next_subnet_id.split(".")[:3] + ((([str(int(next_subnet_id.split(".")[-1]) + hosts_amount)])))
        next_subnet_broadcast = joinList(next_subnet_broadcast)
    elif (int(network_id.split(".")[-1]) == 255):
        if (int(network_id.split(".")[-2]) < (255 - hosts_amount)):
            next_subnet_broadcast = next_subnet_id.split(".")[:2] + ([str(int(next_subnet_id.split(".")[-2]) + 1)]) + str(hosts_amount - 1)
            next_subnet_broadcast = joinList(next_subnet_broadcast)
        elif (int(next_subnet_id.split(".")[-2]) == 255):
            if (int(next_subnet_id.split(".")[-3]) < (255 - hosts_amount)):
                next_subnet_broadcast = next_subnet_id.split(".")[:1] + ([str(int(next_subnet_id.split(".")[-3]) + 1)]) + "0" + str(
                    hosts_amount - 1)
                next_subnet_broadcast = joinList(next_subnet_broadcast)
